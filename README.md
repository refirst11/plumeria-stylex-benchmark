...
|                         |  Build | Library Cost |        CSS |  SSR Chunk | Client Chunk¹ | Class names resolved |
| :---------------------- | -----: | -----------: | ---------: | ---------: | ------------: | :------------------- |
...
Plumeria builds **23% cheaper**, emits a **smaller stylesheet**, and ships **no runtime** — StyleX sends 1.83KB more to the browser once components are client-side. Both score 100/100 on Lighthouse and prerender 1,000 components in an indistinguishable ~210ms.
...
This is where the two libraries actually differ. Both resolve a variant like `color="red"` to an atomic class name, but they place that resolution on opposite sides of the build boundary. The difference is visible in the SSR chunk each emits — `.next/server/chunks/ssr/[root-of-the-server]__*.js`.
...
One literal per variant axis, inlined at the call site and keyed by the variant value. The compiler has already decided which class wins for every reachable combination, so the render does one O(1) property read per axis and concatenates. **No runtime is shipped**, and no merge happens — there is nothing left to merge.
...
|               |  SSR chunk | Class-name structure                                              |   Runtime | Structure + runtime | Merge  | Avg class name |
| :------------ | ---------: | :---------------------------------------------------------------- | --------: | ------------------: | :----- | -------------: |
| _CSS Modules_ |     3.91KB | **1,211B** — name-keyed, 28 entries over two maps                  |         — |          **1,211B** | none   |     29.8 chars |
| **Plumeria**  | **2.60KB** | **885B** — 428B variant lookups + 457B baked class strings         |         — |            **885B** | build  |      8.0 chars |
| **StyleX**    |     4.44KB | **849B** — property-keyed, 25 `$$css` entries in 5 variant objects | **1,258B** |          **2,107B** | render |      7.6 chars |

The inversion in that table is the whole story. **StyleX has the smallest structure of the three** — 849B against Plumeria's 885B, a 36B difference that is really just the shorter hash names. As a data structure the two designs are equivalent.

What separates them is the 1,258B of `styleq` that has to come along to interpret it — **a resolver 1.5x the size of the map it resolves.** Plumeria carries no such thing because the merge already happened, so it ends up 1.4x smaller in total despite the marginally larger map. CSS Modules has the largest structure (its keys and values are both human-readable long names) but still lands under StyleX, having nothing to ship at runtime.

At the whole-chunk level this shows up as **StyleX being 1.84KB larger than Plumeria.**

`styleq` caches merges in a `WeakMap` keyed on the style objects themselves. With a handful of variant objects reused across 1,000 components the cache is warm almost immediately, so the per-render merge cost is not where the two differ — **shipping the resolver is.**

### Where the resolver is paid for

In this benchmark the components are Server Components on a statically prerendered route, so `styleq` runs at build time and never reaches the browser — neither project puts any styling code in `static/chunks`.

That changes the moment a styled component becomes a Client Component, which is the normal case for variant-driven UI. Marking `Test.tsx` with `"use client"` and rebuilding:
...
**StyleX ships 1.83KB more to the browser**, the bulk of it `styleq` itself, and that resolver runs on hydration and on every client re-render. Plumeria has nothing to ship because the strings were resolved during the build — the concatenation is all that survives into the bundle.

This is the practical consequence of the architecture: the map-plus-resolver design is free only while rendering stays on the server **and the route stays static**. `prerender-manifest.json` reports `initialRevalidateSeconds: false` here, so the page renders once at build time and every request is served from the generated HTML. Under ISR or a dynamic route the resolver runs again per regeneration or per request; in a client-rendered React app it runs on every render, always.

Note also that `styleq` is not eliminated by the build even when it never executes — it stays in `.next/server/chunks/ssr/` and is listed among the 113 traced dependencies in `page.js.nft.json`. What is resolved at build time is the *output*, not the code.

---

## Reading the Output Sizes

Two numbers look contradictory at first, and both are misleading without decomposition.

**`.next` totals favour Plumeria (7.18MB vs 7.19MB), but that is not served code.** The advantage is entirely artifacts a user never receives — source maps (17.3KB smaller) and build bookkeeping such as `.nft.json` and `.tsbuildinfo` (25.8KB smaller). Both library projects also carry ~0.89MB of `.next/build/chunks`, where Turbopack bundles the PostCSS config and plugins; the control has no such directory at all.
...
This measures everything adopting the library entails, not just time inside its compiler: loading the plugin packages, running a PostCSS pipeline the control never runs, and — for StyleX — a `babel.config.js` that moves the application source off Next.js's native SWC pipeline onto Babel.

Most of that cost is fixed rather than proportional to the styles compiled. Neither library is doing 500ms of work on two component files; the bulk is toolchain each one brings with it.
...
Next.js 16.2.10 (Turbopack) · React 19.2.4 · StyleX 0.19.0 · Plumeria 16.2.10 · Node v25.8.2 · pnpm 11.3.0 · macOS Tahoe, Apple M1 (8-core), 16GB
...
Runs 10 cold builds of each of the three projects and prints the build table.

```bash
cd baseline-next && npm run build
